...
import { getAdminUsers, createAdminUser, updateAdminUser, inviteAdminUser } from '@/lib/api'
...
function RoleSelector({ value, onChange }: { value: 'member' | 'admin'; onChange: (r: 'member' | 'admin') => void }) {
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {(['member', 'admin'] as const).map(r => (
          <button key={r} type="button" onClick={() => onChange(r)} style={{
            ...
            border: `1px solid ${value === r ? 'var(--text-2)' : 'var(--line-2)'}`,
            background: value === r ? 'var(--bg-3)' : 'transparent',
            color: value === r ? (r === 'admin' ? 'var(--cat-code)' : 'var(--text-0)') : 'var(--text-3)',
...
export default function UsersAdminPage() {
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    setSaving(true); setError(''); setSuccess('')
    try {
      const user = await inviteAdminUser({ email: inviteEmail, role: inviteRole })
      setUsers(prev => [user, ...prev])
    ...
    <div style={{ height: '100%', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ width: 280, borderRight: '1px solid var(--line-2)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--line-2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {selected ? (
            <button onClick={deselect} style={{ background: 'none', border: 'none', color: 'var(--text-3)', fontSize: 11, fontFamily: 'monospace', cursor: 'pointer' }}>+ New</button>
          ) : (
            <div style={{ display: 'flex', gap: 4 }}>
              {(['create', 'invite'] as CreateMode[]).map(m => (
                <button key={m} onClick={() => { setCreateMode(m); setError(''); setSuccess('') }} style={{
                  padding: '3px 10px', borderRadius: 4, fontSize: 10, fontFamily: 'monospace', cursor: 'pointer',
                  border: `1px solid ${createMode === m ? 'var(--text-2)' : 'var(--line-2)'}`,
                  background: createMode === m ? 'var(--bg-3)' : 'transparent',
                  color: createMode === m ? 'var(--text-0)' : 'var(--text-3)',
                }}>{m}</button>
              ))}
            </div>
          )}
          ...
          {selected ? (
            ...
            <form onSubmit={handleInvite} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 11, color: 'var(--text-3)', lineHeight: 1.5 }}>
                An invite email will be sent. The user sets their name and password when they accept.
              </div>
              <div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.auth_id} onClick={() => selectUser(u)}
                  ...
                  >
                    <td style={{ padding: '10px 16px', color: 'var(--text-0)' }}>{u.display_name || <span style={{ color: 'var(--text-3)', fontStyle: 'italic' }}>pending</span>}</td>
