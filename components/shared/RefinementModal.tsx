...
interface Props {
  ...
  imageId:   string
  project:   Project
  onRefined: (result: { image_url: string; refined_from_id: string }) => void
  onClose:   () => void
}
...
function buildStyleContext(project: Project): string {
  ...
  if (!gdd) return ''
  const art  = (gdd.art_direction  as Record<string, string> | undefined) ?? {}
  const proj = (gdd.project        as Record<string, string> | undefined) ?? {}

  const parts: string[] = []
  if (art.style)   parts.push(`Visual style: ${art.style}`)
  if (art.palette) parts.push(`Color palette: ${art.palette}`)
...
export default function RefinementModal({ imageUrl, imageId, project, onRefined, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: '100%', maxWidth: hasResult ? 1300 : 1060, maxHeight: '94vh', background: 'var(--bg-1)', border: '1px solid var(--line-2)', borderRadius: 12, display: 'flex', flexDirection: 'column', boxShadow: '0 32px 80px rgba(0,0,0,0.65)', overflow: 'hidden', transition: 'max-width 200ms ease' }}
      >
        {/* Header */}
          ...
          <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 16, padding: '20px 24px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <img src={imageUrl} alt="original" style={{ width: '100%', borderRadius: 8, border: '1px solid var(--line-2)', objectFit: 'cover', display: 'block' }} />
              </div>
              {/* Result */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
              <button
                onClick={() => { onRefined({ image_url: resultUrl!, refined_from_id: imageId }); onClose() }}
                style={{ flex: 1, padding: '10px 20px', borderRadius: 7, border: 'none', cursor: 'pointer', background: 'var(--cat-code)', color: '#0a0a0c', fontSize: 13, ...mono, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              >
        ...
        {/* ── CONTROLES: edición ── */}
        {!hasResult && (
          <div style={{ flex: 1, overflow: 'auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, padding: '20px 24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {!loadingImg && (!imageFile || !isMask) && (
                <img src={imageUrl} alt="" style={{ width: '100%', borderRadius: 6, border: '1px solid var(--line-2)', objectFit: 'cover', display: 'block' }} />
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button
                onClick={handleRun}
                disabled={running || !selectedWf}
                style={{ padding: '8px 16px', borderRadius: 6, border: 'none', cursor: running || !selectedWf ? 'not-allowed' : 'pointer', background: 'var(--cat-asset)', color: '#0a0a0c', fontSize: 12, ...mono, fontWeight: 700, opacity: running || !selectedWf ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              >
                {running
