    ...
    <section>
      ...
      <div className="flex flex-col gap-3">
        {levels.map((level, i) => (
          ...
          >
            <div
              className={`h-12 w-full bg-gradient-to-r ${DIFF_BAR_COLOR[level.difficulty ?? ''] ?? 'from-slate-900/60 to-slate-800/20'} border-b border-slate-800 flex items-center px-4 gap-3`}
            >
              {level.order != null && <span className="text-slate-400 text-xs font-mono">#{level.order}</span>}
              <span className="font-bold text-white text-sm">{level.name}</span>
              {level.difficulty && (
                <span className={`text-xs px-2 py-0.5 rounded-md border capitalize ${DIFF_STYLES[level.difficulty] ?? ''}`}>
                  {level.difficulty}
                </span>
              )}
            </div>
            <div className="p-4">
              ...
              <p className="text-slate-400 text-xs leading-relaxed">{level.description ?? level.gameplay_role ?? ''}</p>
            </div>
          </div>
        ))}
      </div>
